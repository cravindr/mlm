    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="table-responsive table-responsive-data2">
                <table id="order" width="100%" class="table table-striped table-bordered">
                    <thead>
                    <tr>
                        <th>S.No</th>
                        <th>Number</th>
    ...
    <script>
        $(document).ready(function() {
            $('#order').DataTable({
                ...
                "processing": true,
                "serverSide": true,
                ajax: '{{ URL::to('/dashboard/purchaseorderserverside') }}',
                columns: [
                    { data: 'id' },
                    { data: 'Number' },
                    { data: 'Company_Name' },
                    ...
                    { data: 'Final_Total' },
                    {
                        ...
                        defaultContent: '<button id="btnpreview" title="Preview" type="button" class="btn btn-primary btn-sm">' +
                            ...
                            '<button id="btnprint" type="button" title="Print" class="btn btn-warning btn-sm">' +
            ...
            $('#order tbody').on( 'click', '#btnprint', function () {
                var data = table.row($(this).parents('tr')).data();
                PdfQuote(data.id);
            } );

            $('#order tbody').on( 'click', '#btnpreview', function () {
                var data = table.row($(this).parents('tr')).data();
                Preview(data.id);
            } );
        });
        ...
        function Preview(id) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    html += '<div class="row border mt-5" style="font-size: 13px">'+
                        '<div class="col-8">'+
                        '    <div class="row">'+
                    ...
                    html += '<div class="row mt-3 ml-3">'+
                        '	<div class="col-12">'+
                        '        <h6>Dear Sir/ Madam,</h6>'+
                        '		<p>Please find our best prices quoted for the products mentioned below.</p>'+
                        '    </div>'+
                        '</div>';

                    html += '<div class="row mt-4 no-padding no-margin">';
